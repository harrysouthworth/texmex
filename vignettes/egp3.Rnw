...
for (dose in LETTERS[1:4]){
  plot(egp3RangeFit(liver$r[liver$dose == dose],
                    umin=-.5, umax=.15))
...
ought to fit above a threshold of about 0, the $57^{th}$ percentile, for each
dose.
...
Figure~\ref{fig:liverthresh} and also suggest a threshold of 0 ought to be ok,
...
The GPD model fit to all values above 0 seems to fit ok, the diagnostic plots
...
We now attempt a somewhat higher threshold of -0.25.
