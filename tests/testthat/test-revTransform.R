...
test_that("revTransform behaves as it should", {
  ...
  x.l.m <- cbind(revTransform(u.l.m[, 1], x[, 1], x.fit$mqu[1], x.fit$mth[1],
                 ...
                 revTransform(u.l.m[, 2], x[, 2], x.fit$mqu[2], x.fit$mth[2],
                              exp(x.fit$models[[2]]$coefficients[1]), x.fit$models[[2]]$coefficients[2]))
  ...
  expect_that(x.l.e, equals(x, tolerance=0.0001), label="revTransform: empirical transformation, laplace target")
  expect_that(x.l.m, equals(x, tolerance=0.0001), label="revTransform: mixture transformation, laplace target")
  expect_that(x.g.e, equals(x, tolerance=0.0001), label="revTransform: empirical transformation, gumbel target")
  expect_that(x.g.m, equals(x, tolerance=0.0001), label="revTransform: mixture transformation, gumbel target")
}
)
