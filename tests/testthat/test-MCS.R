...
test_that("MCS behaves as it should", {
  ...
  expect_that(myRl, equals(tmRl$mcs), label="MCS:independentnormaldata")
  expect_that(p, equals(tmRl$p), label="MCS:mathchingpargument")
  
  # winter air pollution data - dimension 5
  tmWinterMCS <- MCS(winter,p)
  myWinterMCS <- myMCS(t(winter),p)
  expect_that(myWinterMCS, equals(tmWinterMCS$mcs), label="MCS:winterairpollutiondata")
  
  # summer airpollution data - dimension 5
  tmSummerMCS <- MCS(summer,p)
  mySummerMCS <- myMCS(t(summer),p)
  expect_that(mySummerMCS, equals(tmSummerMCS$mcs), label="MCS:summerairpollutiondata")
}
)
