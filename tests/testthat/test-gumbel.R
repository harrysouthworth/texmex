...
test_that("gumbel family behaves as it should", {
  ...
  fit1 <- evm(gdat,th=0,family=gumbel,cov="numeric")
  fit2 <- evm(gdat,th=0,family=gumbel)
  ...
  fit1 <- evm(data=df,y=y,mu=~x,th=0,family=gumbel,cov="numeric")
  fit2 <- evm(data=df,y=y,mu=~x,th=0,family=gumbel)
