...
for(Family in list(gpd,gev)){
  ...
  expect_error(plot(p.lp.opt), info=pst("plot.lp.evmOpt: fail if no covariates"))
  expect_error(plot(p.lp.sim), info=pst("plot.lp.evmSim: fail if no covariates"))
  expect_error(plot(p.lp.boot), info=pst("plot.lp.evmBoot: fail if no covariates"))
