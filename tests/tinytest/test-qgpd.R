...
myTest <- function(sig, xi, thresh, msg){
  ...
  expect_equal(eq, myq, info=paste(msg,"testusing.evd.qgpd"))
  expect_equal(x, myp, info=paste(msg,"testusingqgpd"))
}

#*************************************************************
# 6.4.0 Test exception for out of range probabilties
expect_error(qgpd(1.5, 1, 0, 2), info="qgpd:exceptionforoutofrangeprob")
expect_error(qgpd(-1, 1, 0, 2), info="qgpd:exceptionforoutofrangeprob")
...
expect_equal(eq, myq, info="qgpd:vectorisation")
...
expect_equal(myq, lq, info="qgpd:log.p=TRUE")
...
expect_equal(myq, LTq, info="qgpd:lower.tail=FALSE")
