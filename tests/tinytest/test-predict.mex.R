...
expect_equal(Table5summer, resSummer, tolerance=tol, label="predict.mex:Table5summerdata")
expect_equal(Table5winter, resWinter, tolerance=tol, label="predict.mex:Table5winterdata")

#expect_equal(Table5summer, resSummer, tolerance=tol, label="predict.mex:Table5summerdata")
#expect_equal(Table5winter, resWinter, tolerance=tol, label="predict.mex:Table5winterdata")


expect_equal(pointEstSummer, resSummer[1, ], tolerance=tol, label="predict.mex:pointestvsboot,summerdata")
expect_equal(pointEstWinter, resWinter[1, ], tolerance=tol, label="predict.mex:pointestvsboot,winterdata")
...
expect_equal(length(wavesurge.pred$replicates), R, label="predict.mexexecutionfor2-ddata")
expect_equal(dim(wavesurge.pred$replicates[[3]]), c(nsim, 2))
expect_equal(names(wavesurge.pred$replicates[[4]]), names(wavesurge), label="predict.mexexecutionfor2-ddata")
...
for(i in 1:5){
  ...
  expect_equal(lap.ans, gum.ans, tolerance=tol, label=paste("predict.mexLaplacepredictionsequaltoGumbel,testreplicate",i))
}
