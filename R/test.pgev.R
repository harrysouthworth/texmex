...
function() {
  ...
  expect_that(log(pgev(randoms, equals(0),                        pgev(randoms, 0, 1, xi, log.p=TRUE),
                       "pgev: log.p (1)")
  expect_that(log(pgev(randoms, equals(0),                        pgev(randoms, 0, 1, xi, lower.tail=FALSE,
                            log.p=TRUE),
                       "pgev: log.p (2)")
  ...
  expect_that(pgev(randoms, equals(0),                        pgev(mu + sigma * randoms, mu, sigma, xi),
  ...
  expect_that(my.probs, equals(probabilities),     
  ...
  expect_that(probabilities+my.probs.2, equals(),                        rep(1, length(probabilities)),
                       "pgev: tail flip")
  }
