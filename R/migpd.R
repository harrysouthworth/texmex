...
function (data, th, qu, penalty = "gaussian", maxit = 10000,
   verbose = FALSE, gaussianParameters = NULL)
{
   theCall <- match.call()
   d <- dim(data)[2]
   ...
   if (penalty %in% c("quadratic", "gaussian") & missing(gaussianParameters)) {
       gp = list(c(0, 0), matrix(c(100^2, 0, 0, 0.25), nrow = 2))
       gaussianParameters <- vector("list", length = length(th))
       for (i in 1:length(th)) gaussianParameters[[i]] <- gp
       names(gaussianParameters) <- dimnames(data)[[2]]
   }
   if (penalty %in% c("quadratic", "gaussian")) {
       nm <- names(gaussianParameters)
       if (is.null(nm))
           stop("gaussianParameters must be a named list")
       else if (any(!is.element(nm, dimnames(data)[[2]])))
           stop("the names of gaussianParameters must match the column names of the data")
   }
   fitgpd <- function(i, x, th, penalty, maxit, verbose, gaussianParameters) {
       if (verbose)
           cat("Fitting model", i, "\n")
       if (!is.null(gaussianParameters))
           gaussianParameters <- gaussianParameters[[(1:length(gaussianParameters))[names(gaussianParameters) ==
               dimnames(x)[[2]][i]]]]
       x <- c(x[, i])
       th <- th[i]
       x <- x[x > th]
       gpd.lik <- function(par, y, th, penalty = "none", gaussianParameters) {
           sc <- par[1]
           xi <- par[2]
           y <- (y - th)/exp(sc)
           y <- 1 + xi * y
           ...
           if (casefold(penalty) == "none")
               ...
               p <- mahalanobis(matrix(c(sc, xi), nrow = 1),
                 center = gaussianParameters[[1]], cov = gaussianParameters[[2]])
               l <- l + p
           }
...
#        start <- gpd.pwm(x - th)
   ...
   modlist <- lapply(1:d, fitgpd, x = data, penalty = penalty,
       th = th, maxit = maxit, verbose = verbose, gaussianParameters = gaussianParameters)
   ...
   res <- list(call = theCall, models = modlist, data = data,
       th = th, qu = qu, penalty = penalty, gaussianParameters = gaussianParameters)
   res <- cmvxGumbel(res)
   oldClass(res) <- "migpd"
   invisible(res)
}
