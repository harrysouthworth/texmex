...
test_that("mexRangeFit behaves as it should", {
  ...
  expect_that(mexRangeFit(TRUE, equals(which=2)),   expect_that(mexRangeFit(5, equals(which=1)),   
  expect_that(mrf1$ests[[1]][1:2], equals(wmexmod.lap[1:2])),   expect_that(mrf2$ests[[1]][1:2], equals(wmexmod.gum[1:2])),   expect_that(mrf3$ests[[1]][1:2], equals(wmexmod.lap[1:2])),   
  # now 2-d data
