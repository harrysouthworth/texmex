...
function() {
  ...
  expect_that(base.quantiles, equals(),                        qgev(1 - probabilities, 0, 1, xi, lower.tail=FALSE),
                       "qgev: lower.tail works correctly")
    ## does log.p work?
  expect_that(base.quantiles, equals(),                        qgev(log(probabilities), 0, 1, xi, log.p=TRUE),
                       "qgev: log.p works")
    ## check shift and scale property
    sigma <- rexp(1)
    mu    <- runif(1, -5, 5)
    shifted <- mu + sigma * base.quantiles
  expect_that(shifted, equals(),                        qgev(probabilities, mu, sigma, xi),
  ...
  ## known values
  expect_that(-log(log(2)), equals(),                      qgev(0.5, 0, 1, 0),
                     "qgev: median match at zero xi")
  xi <- seq(-2, 2, length=10)
  expect_that(qgev(0.5, equals(0),                      expm1(-log(log(2))*xi) / xi,
