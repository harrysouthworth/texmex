...
test_that("extremalIndex behaves as it should", {
  ...
  expect_that(texmex.ei$EIintervals, equals(Ferro.ei),                        tolerance = tol,label="extremalIndex: extRemes implementation")
  expect_that(texmex.clust$sizes, equals(Ferro.clust$size),                        tolerance = tol,label="extremalIndex: declustering")
    
  expect_that(texmex.runs$nCluster, equals(Ferro.runs$nc),   expect_that(texmex.runs$sizes, equals(Ferro.runs$size),   }
  ...
  expect_that(data.ei$EIintervals, equals(resp.ei$EIintervals),   expect_that(data.de$clusters, equals(resp.de$clusters),   
  ...
  expect_that(662.9508, equals(AIC(evm(d),   expect_that(662.8874, equals(AIC(evm(d),   expect_that(651.8747, equals(AIC(evm(d),   expect_that(663.0015, equals(AIC(evm(d),   expect_that(651.7874, equals(AIC(evm(d),   expect_that(653.2512, equals(AIC(evm(d),   expect_that(653.6385, equals(AIC(evm(d),   expect_that(652.9238, equals(AIC(evm(d),   
}
)
