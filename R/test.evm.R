...
test_that("evm behaves as it should", {
  ...
  expect_that(cparas, equals(mod.coef),                      label="gpd: parameter ests page 85 Coles")
  expect_that(cse[2], equals(mod$se[2]),                      label="gpd: standard errors page 85 Coles")
  expect_that(ccov[2, equals(2]),                      label="gpd: Covariance page 85 Coles")
  expect_that(cloglik, equals(mod.loglik),                      label="gpd: loglik page 85 Coles")
  ...
  expect_that(-484.6, equals(mod$loglik),                      label="gpd: loglik Coles page 119")
  ...
  expect_that(ismod$mle, equals(),                      mco,
                     tolerance=tol,
                     label="gpd: covariates in xi only: point ests")
  # SEs for phi will not be same as for sigma, but we can test xi
  expect_that(ismod$se[-1], equals(),                      mod$se[-1],
                     tolerance = tol,
  ...
  expect_that(ismod$se, equals(),                      sqrt(diag(mod$cov)),
                     tolerance = tol,
  ...
  expect_that(bmod$param, equals(bmod2$param),                      label="gpd: test simulation reproducibility 1")
  
  set.seed(bmod$seed)
  bmod3 <- evm(ALT.M, data=liver,
               th=quantile(liver$ALT.M, .7),
               iter=1000, thin=1, verbose=FALSE, method="sim")
  expect_that(bmod$param, equals(bmod3$param),                      label="gpd: test simulation reproducibility 2")
  
  #*************************************************************
  # 4.2. Logical test of burn-in
  
  expect_that(nrow(bmod$chains)-bmod$burn, equals(nrow(bmod$param)),                      label="gpd: Logical test of burn-in 1")
  ...
  expect_that(iter-burn, equals(nrow(bmod2$param)),                      label="gpd: Logical test of burn-in 2")
  ...
  expect_that((nrow(bmod$chains)-bmod$burn)*thin, equals(nrow(bmod$param)),                      label="gpd: Logical test of thinning 1")
  ...
  expect_that(coles, equals(coOpt),                      label="gev: optimisation, parameter ests page 59 Coles")
  expect_that(coles, equals(coSim),                      label="gev: simulation, parameter ests page 59 Coles")
  ...
  expect_that(coles, equals(coOpt),                      label="gev: optimisation, covariance page 59 coles")
  expect_that(coles, equals(coSim),                      label="gev: simulation, covariance page 59 coles")
  mcOpt <- max(abs(coles - coOpt))
  mcSim <- max(abs(coles - coSim))
  expect_that(0, equals(mcOpt),   expect_that(0, equals(mcSim),   
  # Check log-likelihood
  coles <- 4.34
  co <- mOpt$loglik
  expect_that(coles, equals(co),                      label="gev: optimisation, loglik page 59 coles")
  ...
  expect_that(g1.fit$mle, equals(coef(t1.fit)),   expect_that(g2.fit$mle, equals(coef(t2.fit)),   expect_that(g3.fit$mle, equals(coef(t3.fit)),   
  expect_that(g1.fit$nllh, equals(-t1.fit$loglik),   expect_that(g2.fit$nllh, equals(-t2.fit$loglik),   expect_that(g3.fit$nllh, equals(-t3.fit$loglik),   
  expect_that(g1.fit$cov, equals(t1.fit$cov),   expect_that(g2.fit$cov, equals(t2.fit$cov),   expect_that(g3.fit$cov, equals(t3.fit$cov),   
  ######################################################################
  ...
  expect_that(ismod$mle, equals(coef(mod)),   expect_that(ismod$se, equals(sqrt(diag(mod$cov))),   
  ####################################################################
