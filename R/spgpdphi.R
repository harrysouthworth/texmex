...
spgpd2 <- texmexFamily(name = 'SPGPDphi',
                      log.lik = function(data, th, ...) {
                        ...
                        X.xi <- data$D$xi
                        if (ncol(X.phi) != 2){
                          stop("precisely an intercept and a single predictor in phi are allowed with spgpd2")
                        }
                        n.phi <- ncol(X.phi)
                        n.end <- n.phi + ncol(X.xi)
                        function(param) {
                          stopifnot(length(param) == n.end)
                          param[n.phi] <- exp(param[n.phi])                     ### <---------
                      ...
                      start = function(data){
                        ...
                        X.phi <- data$D[[1]]
                        X.xi <- data$D[[2]]
                        c(log(mean(y)), 1e-05, 1e-05)#rep(1e-05, -1 + ncol(X.phi) + ncol(X.xi)))
                      }, # Close start
